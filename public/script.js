...
if(phoneModeBtn){
  ...
  phoneModeBtn.onclick=()=>{
    ...
    console.log("Telefon modu toggle", isPhoneMode, "wasCam", wasCamOn);
    ...
    if(isPhoneMode){
      ...
      if(phoneCallUI) {
        ...
        const nameEl = document.getElementById('phoneNameDisplay');
        if(nameEl) nameEl.textContent = currentRoom || "Bilinmeyen";
      }
      if(remoteVideo) remoteVideo.style.display="none";
      if(myVideoContainer) myVideoContainer.style.display="none";
      if(candleContainer) candleContainer.classList.remove("show");
      phoneModeBtn.style.background="#00c853";
      socket.emit('phone-mode',true);
    }else{
      if(localStream){ 
        localStream.getVideoTracks().forEach(t=> { t.enabled=wasCamOn; });
        camEnabled=wasCamOn;
        if(wasCamOn) { if(camBtn) camBtn.classList.remove("offIcon"); }
        else { if(camBtn) camBtn.classList.add("offIcon"); }
      }
