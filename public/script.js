...
ait navigator.mediaDevices.getUserMedia({
...
function createPeer(initiator){
    ...
    peer.on("stream", stream=>{
        ...
        if(candleContainer) candleContainer.classList.remove("sho
...
⏳ ${formatTime(remaining)} içinde kaybolacak`; if(remaining<60) countdownEl.style.color="#ff4444"; }
...
ype==="image"){ const im=document.createElement("img"); im.src=dataUrl; im.className="mediaMessage"; im.onclick=(ev)=>{ ev.stopPropagation(); openPreview({type:"image",data:dataUrl,name:fileName}); }; div.appendChild(im); setupLongPress(im, msgId); }
...
image"){ const img=document.createElement("img"); img.src=plain; img.className="mediaMessage"; div.appendChild(img); setupLongPress(img, msgId); }
...
lement("div"); div.className=isMine?"myMessage":"otherMessage"; div.id=m.msgId;
...
socket.on("message-opened",({msgId,deleteAt,expireSec})=>{
    ...
    if(sentMessages.has(msgId)){
        ...
        const cd=div.querySelector(".countdown"); if(cd) c

d.textContent=`⏳ Karşı açtı! ${formatTime(expireSec)}`;
...
if(emojiBtn) emojiBtn.onclick=(e)=>{ e.stopPropagation(); emojiPanel.classList.toggle(
...
function createFlyingEmoji(emoji,effect,isMine){
    ...
    if(effect==='fire' || effect==='wow' || effect==='thumbs'){
        ...
        if(msnEffectLayer){
            ...
            msnEffectLayer.style.background = effect==='fire'? 'radial-gradient(circle at 50% 70%, rgba(255,80,0,0.25), transparent 65%)' : 'radial-gradient(

circle at 50% 50%, rgba(255,255,0,0.2), transparent 60%)';
...
mediaBtn.onclick=(e)=>{ e.prevent

Default(); attachMenu.classList.remove("show"); mediaInput.click(); };
...
mediaInput.onchange=async()=>{
    ...
    try{
      ...
      if(file.size>20*1024*1024){ alert("Max 20MB"); isPickingF

ile=false; _photoPicking=false; return; }
...
('canvas'); const max=1280; let w=img.width,h=img.height; if(w>max){ h=h*max/w; w=max; }
...
drawClear.onclick=()=>{ const ctx=window._drawCtx; if(ctx){ ctx.

fillStyle="#000"; ctx.fillRect(0,0,window.innerWidth,window.innerHeight); } };
...
console.error("saveDisk hata", e.message); }
...
io.on('connection', socket=>{
  ...
  socket.on('chat-media', async data=>{
    ...
    const msg={msgId:data.msgId, enc:data.enc, expireSec:data.expireSec, type:data.mediaType, username:socket.username, realUsername:socket

.realUsername, opened:false, room, expireAt: Date.now()+data.expireSec*1000, deleteAt: null};
