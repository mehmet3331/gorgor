...
qualitySelect.onchange=async()=>{
    ...
    console.log("Kalite degisimi", currentQuality, "wasCam", wasCamOn);
    ...
    // HER GİRİŞTE KAPALI fix'i geri al - onceki durumu koru
    if(localStream){
    ...
    camEnabled = wasCamOn;
    micEnabled = wasMicOn;
    if(wasCamOn) camBtn.classList.remove("offIcon"); else camBtn.classList.add("offIcon");
    if(wasMicOn) micBtn.classList.remove("offIcon"); else micBtn.classList.add("offIcon");
...
socket.on("message-opened",({msgId,deleteAt,expireSec})=>{
    ...
    if(sentMessages.has(msgId)){
        ...
        if(cd) {
            // Sayaç zaten atıldıktan sonra başlamıştı, sadece bilgi güncelle, timer'ı yeniden başlatma
            cd.textContent=`✓ Karşı açtı - ${cd.textContent}`;
            cd.style.color="#00ff88";
        }
        // FIX: Timer zaten atıldıktan sonra başlamıştı, tekrar başlatma - beklemesin
        // startSelfDestruct(div,msgId,expireSec,deleteAt); // KALDIRILDI - sayaç atıldığı anda başlamıştı
    }
});
socket.on("message-opened-ack",({msgId,deleteAt,expireSec})=>{
    const div=document.getElementById(msgId); if(!div) return;
    // FIX: Timer zaten başlamıştı, tekrar başlatma
    const cd=div.querySelector(".countdown"); 
    if(cd){ 
        cd.style.display="block"; 
        // Mevcut kalan süreyi koru, sıfırlama
        // cd.textContent=`⏳ ${formatTime(expireSec)}`; // KALDIRILDI
    }
    // startSelfDestruct(div,msgId,expireSec,deleteAt); // KALDIRILDI - atıldığı anda başlamıştı
});
...
chatToggle.onclick=()=>{
    if(chatPanel.style.display==="flex"){ 
        ...
        chatToggle.textContent="\u2716"; 
        const goBottom = ()=>{ if(messages){ messages.scrollTop = messages.scrollHeight; console.log("GO BOTTOM", messages.scrollHeight); } };
        goBottom();
        ...
        setTimeout(()=>{
          ...
          imgs.forEach(img=>{ img.onload = goBottom; });
          goBottom();
        }, 100);
...
function createFlyingEmoji(emoji,effect,isMine){
    ...
    if(effect==='big-kiss'){ const big=document.createElement('div'); big.className='big-kiss-mark'; big.textContent='💋'; document.body.appendChild(big); setTimeout(()=>big.remove(),2500); return; }
    if(effect==='water'){
        const splash=document.createElement('div'); splash.className='water-drop'; document.body.appendChild(splash); setTimeout(()=>splash.remove(),1200);
        for(let i=0;i<8;i++){ setTimeout(()=>{ const d=document.createElement('div'); d.className='flying-emoji'; d.textContent='💧'; d.style.left=(window.innerWidth/2+Math.random()*200-100)+'px'; d.style.bottom='50%'; d.style.fontSize='40px'; d.style.animation='kissRain 1.5s forwards'; document.body.appendChild(d); setTimeout(()=>d.remove(),1500); }, i*80); }
        return;
    }
    let count=1; if(effect==='heart'||effect==='love'||effect==='kiss') count=8; else if(effect==='kiss-rain') count=12; else if(effect==='flower') count=5; else if(effect==='fire') count=3;
    for(let i=0;i<count;i++){ setTimeout(()=>{ const fly=document.createElement('div'); fly.className='flying-emoji '+(effect||'custom'); if(effect==='kiss'||effect==='kiss-rain') fly.classList.add('kiss-rain'); fly.textContent=emoji; fly.style.left=(startX+Math.random()*180-90+i*15)+'px'; fly.style.bottom=(baseY+Math.random()*40)+'px'; fly.style.fontSize=(effect==='fire'||effect==='wow')?'72px':(58+Math.random()*24)+'px'; document.body.appendChild(fly); setTimeout(()=>fly.remove(),3500); }, i*90); }
...
if(cameraInput){
  cameraInput.onchange=async()=>{
    isPickingFile = true;
    _photoPicking = true;
    console.log("KAMERA FOTO - blok basladi");
    try{
      const file=cameraInput.files[0]; 
      if(!file){ isPickingFile=false; _photoPicking=false; return; }
      ...
      try{
        ...
        const max=1280; let w=img.width,h=img.height; if(w>max){ h=h*max/w; w=max; }
        ...
        console.log("createImageBitmap fail fallback", e);
      ...
      console.log("cameraInput hata engellendi", err); 
    }finally{
      setTimeout(()=>{ isPickingFile=false; _photoPicking=false; console.log("KAMERA blok bitti"); }, 1500);
    }
  };
}
...
if(phoneModeBtn){
    phoneModeBtn.onclick=()=>{
        ...
        if(isPhoneMode){
            ...
            if(localStream){ localStream.getVideoTracks().forEach(t=>t.enabled=false); }
            camEnabled=false; camBtn.classList.add("offIcon");
            phoneCallUI.style.display="none"; if(remoteVideo && remoteVideo.srcObject){ remoteVideo.style.display="block"; remoteVideo.play().catch(()=>{}); } myVideoContainer.style.display="block";
...
if(defaultSelfDestructSelect){ 
  ...
  defaultSelfDestructSelect.addEventListener('touchmove', (e)=>{
    ...
    let diff = Math.abs(diffX) > Math.abs(diffY) ? diffX : diffY; // yatay veya dikey hangisi buyukse
    if(Math.abs(diff) > 15){
      isDraggingExpire=true;
      e.preventDefault();
      let steps = Math.floor(diff/12); // her 12px 1 adim
      let newVal = startExpire + steps*60; // her adim 60sn - saga/yukari artir
      ...
      if(newVal!==defaultExpire){
        ...
        if(defaultSelfDestructSelect.parentElement){
          defaultSelfDestructSelect.parentElement.title=formatTime(defaultExpire)+" - kaydirmali";
        }
      }
    }
  }, {passive:false});
  ...
  defaultSelfDestructSelect.addEventListener('touchend', ()=>{
    if(isDraggingExpire){
      setTimeout(()=>{ isDraggingExpire=false; }, 150);
    }
  });
