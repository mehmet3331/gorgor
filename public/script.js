console.log("V12 PRO - E2E sifreli + acinca sayac + telefon modu");
...
async function decryptText(b64, password) {
    ...
    } catch(e) { return "🔒 Şifre çözülemedi (şifre yanlış olabilir)"; }
}
...
async function startCamera(height = 720, facingMode = currentFacingMode) {
    try {
        ...
        return true;
    } catch (err) { alert("Kamera izni verilmedi, sadece karşı tarafı göreceksin"); return false; }
}
...
function startSelfDestruct(div, msgId, expireSec, deleteAt) {
    ...
    const timeout = setTimeout(() => { div.innerHTML = "💨 Bu mesaj kendini imha etti"; div.className = "selfDestructed"; setTimeout(() => div.remove(), 2000); clearInterval(interval); activeTimers.delete(msgId); }, expireAt - Date.now());
...
function addReduceButton(div, msgId) {
    ...
    btn.onclick = () => {
        ...
        const input = prompt(`Kalan: ${formatTime(remaining)} (max 1 saat)\nYeni süre? 10=10sn, 60=1dk, 300=5dk\nSonuna 's' eklersen kalıcı olur`);
        if (!input) return; let isPersistent = input.toLowerCase().includes('s'); let newVal = parseInt(input.replace(/[^0-9]/g, ''));
        if (isNaN(newVal) || newVal <= 0) { alert("Geçersiz"); return; } if (newVal > MAX_SEC) newVal = MAX_SEC; if (timer && newVal >= remaining) { alert("Kalan süreden daha az yazmalısın!"); return; }
...
async function addMyMessage(text, expireSec) {
    ...
    div.innerHTML = `<span class="expireInfo">🔒 E2E şifreli • ⏰ ${formatTime(expireSec)} içinde kaybolacak (açılınca başlar)</span>BEN → ${linked}<span class="message-tick">✓</span><span class="countdown">⏳ Karşı taraf açınca ${formatTime(expireSec)} sayaç başlayacak</span>`;
...
async function addLockedMessage(msgId, expireSec, enc, mediaType) {
    ...
    const icon = mediaType? "📎" : "💬";
    div.innerHTML = `<span class="expireInfo">🔒 E2E şifreli • Açılmadan server okuyamaz</span>${icon} Yeni gizli ${mediaType || "mesaj"} - ${formatTime(expireSec)} sonra kaybolacak<br><small style="opacity:0.7;">Açınca sayaç başlar (max 1 saat)</small><br><button class="openBtn">🔓 Aç ve sayacı başlat</button><span class="countdown" style="display:none;"></span>`;
    const btn = div.querySelector(".openBtn");
    btn.onclick = async () => {
        btn.textContent = "Açılıyor..."; const plain = await decryptText(enc, currentPassword);
        if (div._mediaType === "text" ||!div._mediaType) {
            ...
            if (div._mediaType === "image") { const img = document.createElement("img"); img.src = plain; img.className = "mediaMessage"; img.onclick = () => openPreview({ type: "image", data: plain, name: "gizli.jpg" }); div.appendChild(img); }
        ...
        div.className = "otherMessage"; startSelfDestruct(div, msgId, expireSec); addReduceButton(div, msgId);
        socket.emit("message-opened", { msgId }); socket.emit("message-read", msgId);
    };
...
socket.on("pending-messages", async (list) => {
    for (const m of list) {
        if (m.opened && m.deleteAt) {
            ...
            div.innerHTML = `<span class="expireInfo">⏰ ${formatTime(remaining)} içinde kaybolacak (önceden açıldı)</span>SEN → ${plain}<span class="countdown">⏳ ${formatTime(remaining)} içinde kaybolacak</span>`;
            messages.appendChild(div); startSelfDestruct(div, m.msgId, remaining, m.deleteAt); addReduceButton(div, m.msgId);
        } else { addLockedMessage(m.msgId, m.expireSec, m.enc, m.type === "text"? "text" : m.type); }
    }
...
socket.on("message-opened", ({ msgId, deleteAt, expireSec }) => {
    ...
    if (sentMessages.has(msgId)) { const tick = div.querySelector(".message-tick"); if (tick) { tick.textContent = "✓✓"; tick.classList.add("read"); } const cd = div.querySelector(".countdown"); if (cd) cd.textContent = `⏳ Karşı taraf açtı! ${formatTime(expireSec)} içinde silinecek`; startSelfDestruct(div, msgId, expireSec, deleteAt); }
});
socket.on("message-opened-ack", ({ msgId, deleteAt, expireSec }) => { const div = document.getElementById(msgId); if (!div) return; const cd = div.querySelector(".countdown"); if (cd) { cd.style.display = "block"; cd.textContent = `⏳ ${formatTime(expireSec)} içinde kaybolacak`; } startSelfDestruct(div, msgId, expireSec, deleteAt); });
socket.on("reduce-accepted", ({ msgId, newExpireSec, newDeleteAt }) => {
    const div = document.getElementById(msgId); if (!div) return; const cd = div.querySelector(".countdown"); if (cd) cd.textContent = `⏳ Süre azaltıldı: ${formatTime(newExpireSec)} içinde kaybolacak`;
...
changePasswordBtn.onclick = () => { const pass = prompt("Yeni şifre"); if (!pass) return; currentPassword = pass; socket.emit("change-password", pass); };
...
mediaInput.onchange = async () => {
    ...
    const MAX_FILE_SIZE = 8 * 1024 * 1024; if (file.size > MAX_FILE_SIZE) { alert("Dosya çok büyük! Max 8MB"); return; }
    let expire = perMessageTimerSelect.value === "default"? defaultExpire : parseInt(perMessageTimerSelect.value); expire = Math.min(expire, MAX_SEC);
    const persistMode = perMessagePersistSelect? perMessagePersistSelect.value : "once";
    if (persistMode === "persist") { defaultExpire = expire; localStorage.setItem("gorgor_default_expire", defaultExpire.toString()); if (defaultSelfDestructSelect) defaultSelfDestructSelect.value = defaultExpire.toString(); }
    ...
    if (file.type.startsWith('image/')) {
        ...
        dataUrl = await new Promise(res => { const reader = new FileReader(); reader.onload = e => res(e.target.result); reader.readAsDataURL(blob); });
    } else { dataUrl = await new Promise(res => { const reader = new FileReader(); reader.onload = e => res(e.target.result); reader.readAsDataURL(file); }); }
    ...
    div.innerHTML = `<span class="expireInfo">🔒 E2E şifreli • ⏰ ${formatTime(expire)} içinde kaybolacak (açılınca başlar)</span>`;
    if (mediaType === "image") { const img = document.createElement("img"); img.src = dataUrl; img.className = "mediaMessage"; img.onclick = () => openPreview({ type: "image", data: dataUrl, name: file.name }); div.appendChild(img); }
...
if (locationBtn) { locationBtn.onclick = async () => { if (!navigator.geolocation) { alert("Konum desteklenmiyor"); return; } navigator.geolocation.getCurrentPosition(async pos => { const url = `https://www.google.com/maps?q=${pos.coords.latitude},${pos.coords.longitude}`; let expire = perMessageTimerSelect.value === "default"? defaultExpire : parseInt(perMessageTimerSelect.value); expire = Math.min(expire, MAX_SEC); const msgId = await addMyMessage("📍 Konumum: " + url, expire); const enc = await encryptText("📍 Konumum: " + url, currentPassword); socket.emit("chat-message", { msgId, enc, expireSec: expire }); }); }; }
if (phoneModeBtn) {
    phoneModeBtn.onclick = () => {
        isPhoneMode =!isPhoneMode; document.body.classList.toggle("phone-mode", isPhoneMode); phoneModeBtn.classList.toggle("active", isPhoneMode);
        if (isPhoneMode) { if (localStream) { localStream.getVideoTracks().forEach(t => t.enabled = false); } camEnabled = false; camBtn.classList.add("offIcon"); phoneCallUI.style.display = "flex"; remoteVideo.style.display = "none"; myVideoContainer.style.display = "none"; if (candleContainer) candleContainer.classList.remove("show"); socket.emit("phone-mode", true); }
        else { if (localStream) { localStream.getVideoTracks().forEach(t => t.enabled = true); } camEnabled = true; camBtn.classList.remove("offIcon"); phoneCallUI.style.display = "none"; if (remoteVideo.srcObject) remoteVideo.style.display = "block"; myVideoContainer.style.display = "block"; socket.emit("phone-mode", false); }
    };
}
socket.on("phone-mode", (enabled) => { if (enabled) { phoneCallUI.style.display = "flex"; remoteVideo.style.display = "none"; if (candleContainer) candleContainer.classList.remove("show"); } else { phoneCallUI.style.display = "none"; if (remoteVideo.srcObject) remoteVideo.style.display = "block"; } });
