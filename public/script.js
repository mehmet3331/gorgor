...
const camBtnTop = document.getElementById("camBtnTop");
if(camBtnTop){
  camBtnTop.onclick = async()=>{
    if(camBtn) camBtn.click();
    else {
      if(!localStream){ try{ await startCamera(currentQuality,currentFacingMode); }catch(e){ return; } }
      camEnabled=!camEnabled;
      localStream.getVideoTracks().forEach(t=>t.enabled=camEnabled);
      camBtnTop.classList.toggle("offIcon",!camEnabled);
    }
  };
}
...
if(lightModeBtn) lightModeBtn.onclick=()=>{
  remoteVideo.classList.toggle("lamp-on");
  remoteVideo.classList.toggle("light-mode");
  lightModeBtn.classList.toggle("active");
  lampOn = remoteVideo.classList.contains("lamp-on");
};
...
if(defaultSelfDestructSelect){
  ...
  defaultSelfDestructSelect.addEventListener('touchmove', (e)=>{
    ...
    let steps = Math.round(diff/8);
    let newVal = startVal + steps*60;
    if(newVal < 10) newVal=10;
    if(newVal > 86400) newVal=86400;
...
document.addEventListener('visibilitychange', ()=>{
  if(window.isPickingFile || typeof isPickingFile!=='undefined' && isPickingFile || _photoPicking){
    console.log("VISIBLE IGNORE - foto seciliyor, atma");
    return;
  }
  if(document.hidden && mainScreen.style.display!=="none"){
    setTimeout(()=>{
      if(window.isPickingFile || typeof isPickingFile!=='undefined' && isPickingFile || _photoPicking) return;
      if(document.hidden && mainScreen.style.display!=="none"){
        if(localStream) localStream.getVideoTracks().forEach(t=>t.enabled=false);
