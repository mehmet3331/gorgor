console.log("V18.9 FINAL FIX - mesaj saat + avatar + tik + genel mod kapanis + oda giris");
...
function updateOpponentDisplay(name,status){ opponentUsername=name||opponentUsername; opponentStatus=status||opponentStatus; if(opponentNameDisplay){ opponentNameDisplay.textContent=opponentUsername||"-"; } if(opponentDot){ opponentDot.className="onlineDot "+(opponentStatus==="varım"||opponentStatus==="online"?"online":"offline"); } if(opponentStatusText){ if(opponentStatus==="varım"||opponentStatus==="online"){ opponentStatusText.textContent="içerde"; opponentStatusText.style.color="#00ff88"; } else { opponentStatusText.textContent="dışarda"; opponentStatusText.style.color="rgba(255,255,255,0.5)"; } } const phoneNameDisplay=document.getElementById("phoneNameDisplay"); if(phoneNameDisplay && opponentUsername){ phoneNameDisplay.textContent=opponentUsername; } }
...
joinBtn.onclick=async()=>{
    ...
    currentRoom=room;
    console.log("Oda giris istegi:", room);
    if(myVideoContainer){ myVideoContainer.style.display="block"; myVideoContainer.style.visibility="visible"; }
    try{ await startCamera(currentQuality); }catch(e){ console.log("Kamera acilamadi ama giriliyor", e); }
...
socket.on("user-connected",(d)=>{ if(!peer) createPeer(false); const oppName=d.username||d.realUsername||"Bilinmeyen"; updateOpponentDisplay(oppName,"varım"); if(candleContainer){ candleContainer.classList.remove("show"); candleContainer.style.display="none"; } });
...
socket.on("user-status",(data)=>{ const {user,status,online}=data; if(user===myRealUsername) return; const isOnline=status==="varım"||online; updateOpponentDisplay(user,isOnline?"varım":"yokum"); if(connectionQuality){ if(isOnline){ connectionQuality.textContent=`${user} içerde`; connectionQuality.className="good"; } else { connectionQuality.textContent=`${user} dışarda`; connectionQuality.className="bad"; } } });
socket.on("user-disconnected",()=>{ if(remoteVideo){ remoteVideo.pause(); try{remoteVideo.srcObject=null;}catch(e){} remoteVideo.removeAttribute("src"); remoteVideo.load(); remoteVideo.style.display="none"; } if(peer){ try{peer.destroy();}catch(e){} peer=null; } if(candleContainer){ candleContainer.classList.add("show"); candleContainer.style.display="flex"; } if(connectionQuality){ connectionQuality.textContent="Karşı yok - Mum 🕯"; connectionQuality.className="bad"; } if(pingTimer){ clearInterval(pingTimer); pingTimer=null; } updateOpponentDisplay(opponentUsername||"Bilinmeyen","yokum"); });
...
sendBtn.onclick=async()=>{ const text=input.value.trim(); if(!text) return; let expire=getExpireFromSelect(); const persistMode=perMessagePersistSelect?perMessagePersistSelect.value:"once"; if(persistMode==="persist"){ defaultExpire=expire; localStorage.setItem("gorgor_default_expire",defaultExpire.toString()); if(defaultSelfDestructSelect) defaultSelfDestructSelect.value=defaultExpire.toString(); } const msgId=await addMyMessage(text,expire,myRealUsername); const enc=await encryptText(text,currentPassword); const sentAt=Date.now(); socket.emit("chat-message",{msgId,enc,expireSec:expire,sentAt}); input.value=""; socket.emit('typing',false); isTyping=false; };
...
camBtn.onclick=async()=>{
  ...
  if(!camEnabled){
    ...
    console.log("Kamera teklif gonderildi - karsi kabul ederse iki taraf acilacak");
  } else {
    camEnabled=false;
    localStream.getVideoTracks().forEach(t=>t.enabled=false);
    camBtn.classList.add("offIcon");
    // Kapatinca iki taraf kapansin istege bagli - simdilik sadece kendim kapat
...
if(phoneModeBtn){
  phoneModeBtn.onclick=()=>{
    ...
    if(isPhoneMode){
      ...
      console.log("Sesli arama teklif gonderildi - karsi kabul ederse iki taraf mic acilacak");
    }else{
      if(localStream){
        localStream.getVideoTracks().forEach(t=>t.enabled=false);
        localStream.getAudioTracks().forEach(t=>t.enabled=false);
...
socket.on("phone-mode",(enabled)=>{
  ...
  if(enabled){
    ...
    // FIX: karsi tarafin mikrofonu da acilsin - otomatik
    if(localStream){
...
drawSend.onclick=async()=>{ const dataUrl=drawCanvas.toDataURL("image/jpeg",0.7); let expire=getExpireFromSelect(); const enc=await encryptText(dataUrl,currentPassword); const msgId=await addMyMediaMessage(dataUrl,"image",expire,"cizim.jpg"); socket.emit("chat-media",{msgId,enc,expireSec:expire,mediaType:"image"}); drawOverlay.style.display="none"; };
